...
from agentic import AgentController, AgentToolRegistry
from agentic.provider import GroqAgentProvider
...
class ProposalAcceptanceError(ValueError):
    """Raised when a proposal cannot be accepted in its current state."""
...
class AgentApprovalError(ValueError):
    """Raised when a client cannot claim an agent action for approval."""
...
def accept_pending_proposal(proposal_id, client_id):
    """Accept one pending proposal and reject its pending competitors atomically.

    Fresh authorization and state checks occur inside the transaction so the
    helper is safe to reuse from a future approved agent action. Notifications
    remain the caller's responsibility and happen only after commit.
    """
    with get_engine().begin() as txn:
        proposal_result = txn.execute(text(
            """
        ...
        proposal = proposal_result.mappings().first()
        proposal = dict(proposal) if proposal else None
        if not proposal:
            raise ProposalAcceptanceError("Proposal not found", 404)
        if proposal["client_id"] != client_id:
            raise ProposalAcceptanceError("Not authorized", 403)
        if proposal["status"] != "pending":
            raise ProposalAcceptanceError("Proposal is no longer pending", 400)

        rejected_result = txn.execute(text(
            """
            SELECT p.freelancer_id, COALESCE(u.full_name, u.username) AS freelancer_name
            FROM proposals p
            JOIN users u ON p.freelancer_id=u.id
            WHERE p.job_id=:job_id AND p.id!=:proposal_id AND p.status='pending'
            """
        ), {"job_id": proposal["job_id"], "proposal_id": proposal_id})
        auto_rejected = [dict(row) for row in rejected_result.mappings().all()]

        txn.execute(text("UPDATE proposals SET status='accepted' WHERE id=:proposal_id"), {
            "proposal_id": proposal_id,
        })
        txn.execute(text(
            "UPDATE proposals SET status='rejected' "
            "WHERE job_id=:job_id AND id!=:proposal_id AND status='pending'"
        ), {"job_id": proposal["job_id"], "proposal_id": proposal_id})
        txn.execute(text("UPDATE jobs SET status='in_progress' WHERE id=:job_id"), {
            "job_id": proposal["job_id"],
        })

    return proposal, auto_rejected


def claim_agent_run_approval(run_id, client_id):
    """Atomically claim a proposed action so it cannot be approved twice."""
    with get_engine().begin() as txn:
        result = txn.execute(text(
            "SELECT * FROM agent_runs WHERE id=:run_id AND client_id=:client_id FOR UPDATE"
        ), {"run_id": run_id, "client_id": client_id})
        run = result.mappings().first()
        run = dict(run) if run else None
        if not run:
            raise AgentApprovalError("Agent run not found", 404)
        action = run.get("proposed_action") or {}
        if isinstance(action, str):
            try:
                action = json.loads(action)
            except json.JSONDecodeError:
                action = {}
        if run["status"] != "pending_approval" or action.get("type") != "accept_pending_proposal":
            raise AgentApprovalError("This run has no pending action to approve", 409)
        txn.execute(text(
            "UPDATE agent_runs SET status='approving' WHERE id=:run_id AND status='pending_approval'"
        ), {"run_id": run_id})
    return action
...
def init_db():
    ...
    with engine.begin() as conn:
        ...
        conn.execute(text("CREATE INDEX IF NOT EXISTS idx_agent_runs_client_created ON agent_runs(client_id, created_at DESC)"))
        conn.execute(text("CREATE INDEX IF NOT EXISTS idx_agent_audit_run_created ON agent_audit_events(run_id, created_at ASC)"))
...
def build_chatbot_system_prompt(user=None):
    role = (user or {}).get("role") if user else None
    role_hint = "guest visitor"
    if role == "client":
        role_hint = "logged-in client"
    elif role == "freelancer":
        role_hint = "logged-in freelancer"
    elif role == "admin":
        role_hint = "platform admin"

    return (
        "You are EscrowIQ Assistant, the built-in helper for the EscrowIQ freelance escrow platform. "
        "Be concise, practical, and product-aware. "
        "Help users understand how EscrowIQ works: registration, email verification, job posting, freelancer matching, "
        "proposal submission, proposal acceptance, funding escrow, submitting work, requesting changes, filing complaints, "
        "admin complaint resolution, notifications, profile updates, and on-platform messaging. "
        "Do not claim that actions were completed unless the user explicitly says they already happened. "
        "Do not invent features that are not clearly part of EscrowIQ. "
        "If asked about payment flow, explain that clients accept one proposal first, then fund escrow for the accepted bid amount. "
        "If asked for development help, you may explain the current project structure at a high level. "
        f"The current user is a {role_hint}. "
        "Keep responses helpful, calm, and safe."
    )


def groq_chat_completion(messages, user=None):
    api_key = chatbot_api_key()
    if not api_key:
        raise RuntimeError("GROQ_API_KEY is not configured on the server yet.")

    try:
        from groq import Groq
    except Exception as exc:
        raise RuntimeError("The Groq SDK is not installed. Add 'groq' to the project dependencies.") from exc

    client = Groq(api_key=api_key)
    response = client.chat.completions.create(
        model=chatbot_model(),
        messages=[{"role": "system", "content": build_chatbot_system_prompt(user)}, *messages],
        temperature=0.5,
...
def agent_max_steps():
    try:
        return max(1, min(int(os.environ.get("AGENT_MAX_STEPS", "4")), 6))
    except ValueError:
        return 4


def groq_agent_completion(messages):
    """Structured Groq completion seam kept separate from the chatbot."""
    return GroqAgentProvider()(messages)


def agent_audit(run_id, event_type, payload):
    mutate_db(
        "INSERT INTO agent_audit_events (run_id, event_type, payload) VALUES (?, ?, CAST(? AS jsonb)) RETURNING id",
...
def public_agent_run(run):
    ...
    return {
        ...
        "summary": run.get("summary") or "", "proposed_action": action,
        "error_message": run.get("error_message") or "", "created_at": iso_datetime(run.get("created_at")),
        "completed_at": iso_datetime(run.get("completed_at")),
    }
...
def agent_workspace_page():
    ...
    if user["role"] != "client":
        return redirect(url_for("dashboard"))
    return render_template("agent_workspace.html", user=user)
...
def api_accept_proposal(proposal_id):
    user = current_user()
    try:
        proposal, auto_rejected = accept_pending_proposal(proposal_id, user["id"])
    except ProposalAcceptanceError as exc:
        return jsonify({"error": str(exc)}), exc.status_code

    send_proposal_acceptance_notifications(proposal, auto_rejected)
    return jsonify({"message": "Proposal accepted"}), 200


def send_proposal_acceptance_notifications(proposal, auto_rejected):
    """Keep human and approved-agent acceptance communications identical."""
    notify_and_email(
...
def api_chatbot():
    ...
    if not messages or messages[-1]["role"] != "user":
        return jsonify({"error": "The last message must be a user message"}), 400

    try:
        reply = groq_chat_completion(messages, user=current_user())
    except RuntimeError as exc:
        return jsonify({"error": str(exc)}), 503
    except Exception:
        return jsonify({"error": "Chatbot request failed. Please try again."}), 500

    return jsonify({"reply": reply, "model": chatbot_model()}), 200
...
def api_create_agent_run():
    user = current_user()
    if user["role"] != "client":
        return jsonify({"error": "Only clients can use the hiring assistant"}), 403
    prompt = (get_json_safe().get("prompt") or "").strip()
    if not 10 <= len(prompt) <= 2000:
        return jsonify({"error": "Describe your hiring need in 10 to 2000 characters"}), 400

    run_id = mutate_db(
        "INSERT INTO agent_runs (client_id, prompt) VALUES (?, ?) RETURNING id", [user["id"], prompt]
    )
    agent_audit(run_id, "run_started", {})
    registry = AgentToolRegistry(query_db, hybrid_match_freelancers)
    controller = AgentController(groq_agent_completion, max_steps=agent_max_steps())
    try:
        result = controller.run(prompt, user["id"], registry, lambda event, payload: agent_audit(run_id, event, payload))
        status = "pending_approval" if result["proposed_action"] else "completed"
        mutate_db(
            "UPDATE agent_runs SET status=?, summary=?, proposed_action=CAST(? AS jsonb), completed_at=CURRENT_TIMESTAMP WHERE id=?",
            [status, result["summary"], json.dumps(result["proposed_action"] or {}), run_id],
        )
        agent_audit(run_id, "run_completed", {"status": status})
    except Exception as exc:
        message = str(exc) if isinstance(exc, RuntimeError) else "The hiring assistant could not complete this run."
        mutate_db(
            "UPDATE agent_runs SET status='failed', error_message=?, completed_at=CURRENT_TIMESTAMP WHERE id=?",
            [message[:500], run_id],
        )
        agent_audit(run_id, "run_failed", {})
        return jsonify({"error": message, "run_id": run_id}), 503

    run = query_db("SELECT * FROM agent_runs WHERE id=? AND client_id=?", [run_id, user["id"]], one=True)
    return jsonify({"run": public_agent_run(run)}), 201
...
def api_get_agent_run(run_id):
    ...
    events = query_db(
        "SELECT event_type, created_at FROM agent_audit_events WHERE run_id=? ORDER BY id ASC", [run_id]
    )
    return jsonify({"run": public_agent_run(run), "events": [
        {"event_type": event["event_type"], "created_at": iso_datetime(event["created_at"])} for event in events
    ]}), 200


@app.route("/api/agent/runs/<int:run_id>/cancel", methods=["POST"])
@login_required
def api_cancel_agent_run(run_id):
    ...
    if not run:
        return jsonify({"error": "Agent run not found"}), 404
    cancelled = mutate_db(
        "UPDATE agent_runs SET status='cancelled', cancelled_at=CURRENT_TIMESTAMP "
        "WHERE id=? AND client_id=? AND status='pending_approval' RETURNING id",
        [run_id, user["id"]],
    )
    if not cancelled:
        return jsonify({"error": "Only a pending approval can be cancelled"}), 409
    agent_audit(run_id, "approval_cancelled", {})
    return jsonify({"message": "Proposed action cancelled"}), 200


@app.route("/api/agent/runs/<int:run_id>/approve", methods=["POST"])
@login_required
def api_approve_agent_run(run_id):
    user = current_user()
    if user["role"] != "client":
        return jsonify({"error": "Only clients can approve hiring actions"}), 403
    try:
        action = claim_agent_run_approval(run_id, user["id"])
    except AgentApprovalError as exc:
        return jsonify({"error": str(exc)}), exc.status_code
    try:
        proposal, auto_rejected = accept_pending_proposal(action.get("proposal_id"), user["id"])
    except ProposalAcceptanceError as exc:
        mutate_db(
            "UPDATE agent_runs SET status='stale', error_message=? WHERE id=? AND status='approving'",
            [str(exc), run_id],
        )
        agent_audit(run_id, "approval_stale", {})
        return jsonify({"error": "The proposed action is no longer valid. Please run the assistant again."}), 409

    send_proposal_acceptance_notifications(proposal, auto_rejected)
    mutate_db(
        "UPDATE agent_runs SET status='approved', completed_at=CURRENT_TIMESTAMP WHERE id=? AND status='approving'",
        [run_id],
    )
    agent_audit(run_id, "approval_completed", {"proposal_id": proposal["id"]})
    return jsonify({"message": "Proposal accepted", "proposal_id": proposal["id"]}), 200
