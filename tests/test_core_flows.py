...
class EscrowIQFlowTests(unittest.TestCase):
    def setUp(self):
        ...
        def fake_deliver_email(recipient, subject, text_body, html_body=None):
            self.sent_emails.append(
                {"recipient": recipient, "subject": subject, "body": text_body, "html_body": html_body}
            )
    ...
    def tearDown(self):
        ...
        for email in self.test_emails:
            ...
            self.app_module.mutate_db(
                "DELETE FROM agent_audit_events WHERE run_id IN "
                "(SELECT id FROM agent_runs WHERE client_id IN (SELECT id FROM users WHERE email=?))",
                [email],
            )
            self.app_module.mutate_db(
                "DELETE FROM agent_runs WHERE client_id IN (SELECT id FROM users WHERE email=?)", [email]
            )
    ...
    def register_user(self, username, email, role, full_name=None, skills="", bio=""):
        response = self.client.post(
            ...
            json={
                ...
                "password": "Secret123",
                "role": role,
                "skills": skills or ("Python, Flask" if role == "freelancer" else ""),
                "bio": bio,
            },
    ...
    def test_accept_pending_proposal_helper_updates_one_job_atomically(self):
        client_email = self.email("helperclient")
        freelancer_a_email = self.email("helperfreelancera")
        freelancer_b_email = self.email("helperfreelancerb")
        self.register_user("helperclient_" + self.run_id, client_email, "client")
        self.register_user("helperfreelancera_" + self.run_id, freelancer_a_email, "freelancer")
        self.register_user("helperfreelancerb_" + self.run_id, freelancer_b_email, "freelancer")

        self.login_user(client_email)
        job_id = self.post_job(title="Proposal helper test")
        self.logout_user()

        self.login_user(freelancer_a_email)
        proposal_a = self.submit_proposal(job_id, bid_amount=610)
        self.logout_user()
        self.login_user(freelancer_b_email)
        proposal_b = self.submit_proposal(job_id, bid_amount=620)
        self.logout_user()

        client_id = self.app_module.query_db(
            "SELECT id FROM users WHERE email=?", [client_email], one=True
        )["id"]
        proposal, rejected = self.app_module.accept_pending_proposal(proposal_a, client_id)

        self.assertEqual(proposal["id"], proposal_a)
        self.assertEqual([row["freelancer_id"] for row in rejected], [
            self.app_module.query_db(
                "SELECT freelancer_id FROM proposals WHERE id=?", [proposal_b], one=True
            )["freelancer_id"]
        ])
        statuses = self.app_module.query_db(
            "SELECT id, status FROM proposals WHERE id IN (?, ?) ORDER BY id", [proposal_a, proposal_b]
        )
        self.assertEqual({row["id"]: row["status"] for row in statuses}, {
            proposal_a: "accepted", proposal_b: "rejected"
        })
        self.assertEqual(
            self.app_module.query_db("SELECT status FROM jobs WHERE id=?", [job_id], one=True)["status"],
            "in_progress",
        )
        with self.assertRaisesRegex(self.app_module.ProposalAcceptanceError, "no longer pending"):
            self.app_module.accept_pending_proposal(proposal_b, client_id)

    def test_agent_run_uses_fake_groq_seam_and_keeps_actions_pending(self):
        client_email = self.email("agentclient")
        self.register_user("agentclient_" + self.run_id, client_email, "client")
        self.login_user(client_email)
        self.post_job(title="Agent workspace job")

        original_completion = self.app_module.groq_agent_completion
        calls = []

        def fake_completion(messages):
            calls.append(messages)
            if len(calls) == 1:
                return '{"tool":"list_client_jobs","arguments":{}}'
            return '{"summary":"Your open job has been reviewed. No proposal acceptance is recommended yet.","propose_acceptance":null}'

        self.app_module.groq_agent_completion = fake_completion
        try:
            response = self.client.post(
                "/api/agent/runs",
                json={"prompt": "Review my active job and advise on the next hiring step."},
                headers=self.csrf_headers(),
            )
        finally:
            self.app_module.groq_agent_completion = original_completion

        self.assertEqual(response.status_code, 201, response.get_json())
        run = response.get_json()["run"]
        self.assertEqual(run["status"], "completed")
        self.assertEqual(run["proposed_action"], {})
        self.assertEqual(len(calls), 2)
        read_response = self.client.get(f"/api/agent/runs/{run['id']}")
        self.assertEqual(read_response.status_code, 200, read_response.get_json())
        self.assertTrue(any(event["event_type"] == "tool_used" for event in read_response.get_json()["events"]))

    def test_agent_approval_is_single_use_and_revalidates_proposal_state(self):
        client_email = self.email("approvalclient")
        freelancer_email = self.email("approvalfreelancer")
        self.register_user("approvalclient_" + self.run_id, client_email, "client")
        self.register_user("approvalfreelancer_" + self.run_id, freelancer_email, "freelancer")
        self.login_user(client_email)
        job_id = self.post_job(title="Agent approval job")
        self.logout_user()
        self.login_user(freelancer_email)
        proposal_id = self.submit_proposal(job_id, bid_amount=650)
        self.logout_user()
        self.login_user(client_email)

        client_id = self.app_module.query_db("SELECT id FROM users WHERE email=?", [client_email], one=True)["id"]
        run_id = self.app_module.mutate_db(
            """
            INSERT INTO agent_runs (client_id, prompt, status, proposed_action)
            VALUES (?, ?, 'pending_approval', CAST(? AS jsonb)) RETURNING id
            """,
            [client_id, "Approve the recommended proposal.", '{"type":"accept_pending_proposal","proposal_id":%d}' % proposal_id],
        )
        approved = self.client.post(f"/api/agent/runs/{run_id}/approve", headers=self.csrf_headers())
        self.assertEqual(approved.status_code, 200, approved.get_json())
        self.assertEqual(
            self.app_module.query_db("SELECT status FROM proposals WHERE id=?", [proposal_id], one=True)["status"],
            "accepted",
        )
        repeated = self.client.post(f"/api/agent/runs/{run_id}/approve", headers=self.csrf_headers())
        self.assertEqual(repeated.status_code, 409, repeated.get_json())

        stale_run_id = self.app_module.mutate_db(
            """
            INSERT INTO agent_runs (client_id, prompt, status, proposed_action)
            VALUES (?, ?, 'pending_approval', CAST(? AS jsonb)) RETURNING id
            """,
            [client_id, "Try the old proposal.", '{"type":"accept_pending_proposal","proposal_id":%d}' % proposal_id],
        )
        stale = self.client.post(f"/api/agent/runs/{stale_run_id}/approve", headers=self.csrf_headers())
        self.assertEqual(stale.status_code, 409, stale.get_json())
        self.assertEqual(
            self.app_module.query_db("SELECT status FROM agent_runs WHERE id=?", [stale_run_id], one=True)["status"],
            "stale",
        )

    def test_agent_cancel_and_run_ownership_protections(self):
        owner_email = self.email("agentowner")
        other_email = self.email("agentother")
        self.register_user("agentowner_" + self.run_id, owner_email, "client")
        self.register_user("agentother_" + self.run_id, other_email, "client")
        owner_id = self.app_module.query_db("SELECT id FROM users WHERE email=?", [owner_email], one=True)["id"]
        run_id = self.app_module.mutate_db(
            "INSERT INTO agent_runs (client_id, prompt, status) VALUES (?, ?, 'pending_approval') RETURNING id",
            [owner_id, "Cancel this proposal recommendation."],
        )
        self.login_user(owner_email)
        cancelled = self.client.post(f"/api/agent/runs/{run_id}/cancel", headers=self.csrf_headers())
        self.assertEqual(cancelled.status_code, 200, cancelled.get_json())
        self.logout_user()
        self.login_user(other_email)
        self.assertEqual(self.client.get(f"/api/agent/runs/{run_id}").status_code, 404)
        self.assertEqual(
            self.client.post(f"/api/agent/runs/{run_id}/approve", headers=self.csrf_headers()).status_code,
            404,
        )
    ...
    def test_proposal_and_payment_events_create_notifications_and_emails(self):
        ...
        expected_subjects = {
            ...
            (freelancer_email, "Proposal accepted —"),
            (client_email, "Escrow funded —"),
            (freelancer_email, "Escrow funded —"),
    ...
    def test_reject_and_refund_send_notifications_emails_and_update_job(self):
        ...
        self.assertTrue(any(msg["recipient"] == freelancer_b_email and msg["subject"].startswith("Proposal update —") for msg in self.sent_emails))
